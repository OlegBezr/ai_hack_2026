// @ts-nocheck — vendored third-party StPageFlip (patched). Not typechecked by this app.
    ...
    private onTouchStart = (e: TouchEvent): void => {
        if (this.checkTarget(e.target)) {
            if (e.changedTouches.length > 0) {
                ...
                // Grab the page immediately on touch, exactly like a mouse
                // press (onMouseDown), so the corner follows the finger from
                // the first move. Previously this was deferred by `swipeTimeout`
                // ms, which meant a natural finger drag did nothing until you
                // pressed-and-held — making interactive flipping feel
                // mouse-only on iPad/iPhone. Scroll-vs-flip is still
                // disambiguated by the `dx > 10` gate in onTouchMove, and quick
                // flicks are still caught by the swipe check in onTouchEnd.
                this.app.startUserTouch(pos);
