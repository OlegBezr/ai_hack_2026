  ...
  BookPage? getBottomPage(FlipDirection direction) {
    ...
    if (render.getOrientation() == BookOrientation.portrait) {
      ...
      final spread = direction == FlipDirection.forward
          ? getSpread()[current + 1]
          : getSpread()[current - 1];
      // Single-page spreads (cover / trailing odd page) leave one side blank;
      // returning the page here duplicates the flipping leaf underneath itself.
      if (spread.length == 1) return null;
      return direction == FlipDirection.forward
          ? pages[spread[1]]
          : pages[spread[0]];
    }
